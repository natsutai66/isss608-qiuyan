...
```{r}
pacman::p_load(tidyverse, jsonlite,tidygraph, dplyr,
               ggraph, ggrepel, SmartEDA, visNetwork, 
...
```{r}
#| echo: false
# 1 Re-extract Nadia’s 2-hop ego 
nadia_index <- which(mc3_nodes_final$name == "Nadia Conti")
nadia_ego <- graph2 %>%
  activate(nodes) %>%
  filter(dist <= 2) %>%
...
# 2 Build nodes_vis, forcing `group` to character
nodes_vis <- nadia_ego %>%
  activate(nodes) %>%
  as_tibble() %>%
  mutate(
...
visNetwork(nodes_vis, edges_vis, width="100%", height="600px") %>%
  visGroups(groupname="Person",       color="skyblue",    shape="dot")     %>%
  visGroups(groupname="Event",        color="orange",     shape="square")  %>%
  visGroups(groupname="Location",     color="lightgreen", shape="triangle")%>%
  visGroups(groupname="Relationship", color="plum",       shape="diamond")%>%
  visEdges(color="gray", smooth=FALSE) %>%
  visOptions(
...
# 3 Get event indices connected to Nadia
nadia_event_indices <- edges_involving_nadia %>%
  mutate(event_index = if_else(from == nadia_id, to, from)) %>%
  distinct(event_index) %>%
  pull(event_index)

# 4 Extract Nadia's events and timestamps
nadia_events <- mc3_nodes_final %>%
  slice(nadia_event_indices) %>%
  filter(type == "Event") %>%
  mutate(timestamp = ymd_hms(timestamp)) %>%
...
# 5 Identify partner entity for each event
get_partner_name <- function(event_index) {
  edges <- mc3_edges_final %>%
    filter((from == event_index & to != nadia_id) |
           (to == event_index & from != nadia_id))
...
# 6 Add partner and inferred flag
nadia_events <- nadia_events %>%
  rowwise() %>%
  mutate(
    partner = get_partner_name(new_index),
...
# 7 Add manually confirmed illegal activities
nadia_events <- nadia_events %>%
  add_row(
    id = "Rel_AccessPermit_NemoReef", 
    timestamp = ymd_hms("2040-10-05 00:00:00"),
    sub_type = "AccessPermission", 
    name = NA, content = "Special permit granted to Nemo Reef outside formal review process", 
    partner = "Nemo Reef", inferred = TRUE
  ) %>%
  add_row(
    id = "Rel_Payment_Elise", 
    timestamp = ymd_hms("2040-10-10 05:00:00"),
    sub_type = "Payment", 
    name = NA, content = "Covert funding for illegal reef operation provided by Elise", 
    partner = "Elise", inferred = TRUE
  )
...
timeline_df <- nadia_events %>%
  mutate(
    ...
    confirmed_flag = if_else(event_type %in% c("AccessPermission", "Bribery", "Payment"),
                             "Confirmed Illegal", "Inferred/Suspicious"),
    shape_code = if_else(confirmed_flag == "Confirmed Illegal", "triangle-up", "circle"),
    color_code = event_colors[event_type],
    ...
    tooltip = paste0(
      ...
      "<b>Timestamp:</b> ", format(timestamp, "%Y-%m-%d %H:%M"), "<br>",
      ifelse(wrapped_evidence != "", paste0("<b>Evidence:</b> ", wrapped_evidence), "")
    ),
    hour_group = if_else(event_type == "Communication", floor_date(timestamp, "hour"), NA_POSIXct_),
    y_base = as.numeric(factor(event_type, levels = event_levels))
  ) %>%
  group_by(event_type, hour_group) %>%
  mutate(
    comm_index = if_else(event_type == "Communication", row_number(), NA_integer_),
    n_comm = if_else(event_type == "Communication", n(), NA_integer_),
    comm_offset = if_else(event_type == "Communication",
                          (comm_index - (n_comm + 1) / 2) * 0.12,
                          0),
    y_jittered = y_base + comm_offset
  ) %>%
  ungroup()

# 11 Plot (labels hidden for Communication only)
plot_ly(timeline_df,
        x = ~timestamp,
        y = ~y_jittered,
        type = 'scatter',
        mode = 'markers+text',
        text = ~if_else(event_type == "Communication", "", partner),
        textposition = ~if_else(event_type == "Communication", NA_character_, "top center"),
        hovertext = ~tooltip,
        marker = list(size = 11),
        color = ~I(color_code),
        symbol = ~shape_code,
  ...
  layout(
    title = list(
      text = "Interactive Timeline of Nadia Conti’s Activities",
      y = 0.95
    ),
