...
### Betweenness of Nadia

In the event-based ego network, the largest orange nodes—most notably the “Nemo Reef” communication event and key transmissions at “Remora”—sit at the highest betweenness scores just one hop from Nadia. This indicates that Nadia is the primary information conduit: she channels a disproportionate share of critical messages about reef activity, positioning her as the indispensable broker in every major operational exchange.

In the relationship-based ego network, the most central purple nodes correspond to her AccessPermission and Suspicious ties linking Nadia to “Nemo Reef” itself and to figures like Davis. Here she emerges as the core conspiracy facilitator, bridging otherwise disconnected vessels, officials, and intermediaries through permits and bribes. Taken together, these two views reveal Nadia’s dual role—carrying sensitive event-level intelligence and arranging the illicit permissions—that underpins her centrality in the reef exploitation scheme.
...
graph2 <- tbl_graph(
  ...
  activate(nodes) %>%
  mutate(
    dist = node_distance_from(nadia_index),
    node_cat = case_when(
      sub_type == "Person"                         ~ "Person",
      sub_type %in% c("Location","Vessel")         ~ "Location",
      sub_type %in% c("Communication","Monitoring","VesselMovement") 
  ...
  activate(edges) %>%
  mutate(
    edge_cat = mc3_nodes_final$sub_type[to]
  )

plot_betweenness_ego <- function(ego_graph, title_text) {
  ego_graph %>%
    activate(nodes) %>%
    mutate(betw = centrality_betweenness(normalized = TRUE)) %>%
    ggraph(layout = "fr") +
    
    geom_edge_link(color = "gray80", width = 0.5, alpha = 0.7) +
    
    geom_node_point(aes(size = betw, fill = node_cat),
                    shape = 21, color = "black") +
    
    geom_node_text(aes(label = ifelse(node_cat != "Other", name, "")),
                   repel = TRUE, size = 3) +
    ...
    scale_fill_manual(
      ...
      guide = guide_legend(
        ...
        override.aes    = list(
          ...
          size   = 6,        # larger symbol
          colour = "black"   # border for contrast
        )
      )
    ) +
...
event_ego <- graph2 %>%
  ...
  filter(
    dist <= 2,
    (dist == 0) |
    (dist == 1 & node_cat == "Event") |
    (dist == 2)
  )

plot_betweenness_ego(
  event_ego,
  title_text = "Betweenness: Nadia → Event"
)
```
...
rel_ego <- graph2 %>%
  ...
  filter(
    dist <= 2,
    (dist == 0) |
    (dist == 1 & node_cat == "Relationship") |
    (dist == 2)
  )

plot_betweenness_ego(
  rel_ego,
  title_text = "Betweenness: Nadia → Relationship"
)
```
...
The visual investigation confirms that Nadia Conti occupies a uniquely central position in both the event and relationship spheres of the MC3 network. In the event-based betweenness plot, the Nemo Reef communication and the Remora vessel transmissions emerge as the highest‐traffic hubs, underscoring her function as the indispensable messenger. In the relationship-based view, her AccessPermission and Suspicious ties to Nemo Reef and to key actors like Davis and Liam Thorne rise to the top, revealing her as the principal architect of permits and payoffs. The timeline then situates these insights in time—beginning with the October 5 permit, spiking with October 9’s bribery arrangement, and flowing into October 10 payments—while dense clusters of communication events mark moments of synchronized operational coordination. Taken together, the static and interactive outputs not only validate Clepper Jessen’s concerns but also surface hidden patterns (e.g., synchronized morning intercepts, overlapping vessel-movement and permit sign-offs) that merit deeper forensic scrutiny.
